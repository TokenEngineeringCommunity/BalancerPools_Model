...
with events as (
    ...
    to_address as pool, swapFee
    from `blockchain-etl.ethereum_balancer.BPool_call_setSwapFee`
    where status=1
),
...
state_with_gaps as (
    select events.block_number, events.pool, events.swapFee,
...
running_state as (
    select pool, calendar.block_number, swapFee
...
select pool as address, block_number, swapFee
from running_state where pool='{}' order by block_number
